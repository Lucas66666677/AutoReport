...
| 垃圾桶 | 防止误删与孤儿檔 | 删除／恢复／永久删除；清理 outbox／版本／Yjs | owner-checked permanent-delete API | is_trashed＋两個 private buckets | Owner only＋service role cleanup | 前端 outbox＋后端 cleanup 回归 | Guest 路径已看 | Implemented but unverified | 云端多账号与 Storage 清理 E2E |
...
| AI 配额 | 防滥用与成本失控 | 显示额度 | reserve／refund RPC | profiles＋RPC | service role only | SQL 静态审查＋staging runner | 未做真实 DB | Implemented but unverified | clean staging 验证 |
...
| Owner／Editor／Viewer | 小组分工 | 协作者管理 UI | Supabase | collaborators | RLS＋trigger | SQL 静态测试＋真实请求 runner | 无多账号 | Implemented but unverified | staging 执行三账号权限矩阵 |
...
| Profile 保护 | 用户不能改方案／额度 | 无直接入口 | service RPC | trigger | service fields protected；client insert 已移除 | SQL 回归＋staging runner | 无 staging | Implemented but unverified | clean DB 真实请求验证 |
...
| Clean DB 初始化 | 防迁移上线失败 | 不适用 | 不适用 | 9 个有序 migration | RLS | bootstrap 同步测试＋全部 SQL parse | 未做真实 DB | Implemented but unverified | Owner 建立 staging 后 dry-run／push／catalog 验证 |
