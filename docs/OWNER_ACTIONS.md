...
| Supabase SQL（AI app 最小权限） | 已上线的项目在 SQL Editor 执行 `supabase/migrations/20260918_ai_app_least_privilege.sql` 一次 | SQL Editor | 仓库里的该文件内容（没有任何 secret） | AI app（ChatGPT 网页版）拿到的授权原本等同学生本人；执行后只能读报告、建立私人报告、改报告内容、新增版本备份，不能删除、分享、搬移，也碰不到其他资料与图片 | `https://autoreport-xnq5.onrender.com/api/mcp/status` 的 `ai_app_limits` 变成 `true`；同意页改显示「不能刪除、分享或搬移報告」 | 否；开放 ChatGPT 网页版前建议先执行 |
