...
|---|---|---|---|---|---|---|
| Supabase Database | 在全新项目一次贴上 `supabase/bringup.sql`（等同按顺序执行全部 12 个 migration，至 `20260918_ai_app_least_privilege.sql`）| Project Settings／CLI Migration 管理 | Project ref 与本机保存的数据库密码；不要贴到聊天 | 建立 RLS、trigger、quota RPC 与私有 Storage | dry-run 清单一致、push 成功并完成下方权限矩阵 | 是 |
...
| Supabase OAuth Server | 开启 OAuth 2.1 server，Authorization Path 设为 `/oauth/consent`，并允许动态注册（Dynamic Client Registration） | Authentication → OAuth Server | Authorization Path `/oauth/consent`；Site URL 须为 `https://autolabreport.lucirel.com` | ChatGPT、Claude、Gemini 的网页版都只能连云端 MCP（后端 `/mcp`），它们的登录授权都由 Supabase OAuth 提供；电脑上的 Claude Desktop／Claude Code／ChatGPT 桌面版／Codex／Gemini CLI 走本机连接器，不需要这一步 | `https://<project>.supabase.co/.well-known/oauth-authorization-server/auth/v1` 返回 200 且含 `registration_endpoint`；连接器面板出现「用網頁版 AI（ChatGPT、Claude、Gemini）」步骤 | 否；开启后才有网页版 AI（ChatGPT、Claude、Gemini） |
| Supabase SQL（AI app 最小权限） | 已上线的项目在 SQL Editor 执行 `supabase/migrations/20260918_ai_app_least_privilege.sql` 一次 | SQL Editor | 仓库里的该文件内容（没有任何 secret） | 网页版 AI app（ChatGPT、Claude、Gemini）拿到的授权原本等同学生本人；执行后只能读报告、建立私人报告、改报告内容、新增版本备份，不能删除、分享、搬移，也碰不到其他资料与图片 | `https://autoreport-xnq5.onrender.com/api/mcp/status` 的 `ai_app_limits` 变成 `true`；同意页改显示「不能刪除、分享或搬移報告」 | 否；开放网页版 AI 前建议先执行 |
| Render（可选） | 加 `SUPABASE_ANON_KEY`，值为公开的 anon key（不是 secret） | Service → Environment | 与 Vercel 的 `VITE_SUPABASE_ANON_KEY` 相同 | `/mcp` 以学生身份查资料库时，网关改用 anon key：万一网关忽略学生 token，也只会以 anon 执行，而不是 service role | 网页版 AI 的工具照常运作 | 否 |
